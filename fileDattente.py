...
class Client:
	...
	"""
		Méthode publique d'auto-insertion du client courant dans la file d'attente. La téte de file lui est transmise en paramétre
	"""
	def insertIntoQueue(self, headOfTheQueue):
		# S'il n'y a personne en téte de file. Autrement dit la file est vide
		if(headOfTheQueue == None):
			return self # La cliente devient la téte de file. Et c'est terminé.
		...
		# ... et on se met derriére lui
		currentClient.nextClient = self
		self.previousClient = currentClient
		# On informe l'instance de la file d'attente de la téte de file retenue aprés traitement
	...
	"""
	def show(self):
		# On récupére le numéro du client courant
		text = "Né " + str(self.number) + " (" + self.arrival + ")\n"
		# Et on demande é l'éventuel client qui suit dans la file d'attente de s'afficher
		if(self.nextClient != None):
			text += self.nextClient.show()
		return text
	
	def __str__(self):
		return "Né " + str(self.number) + " (" + self.arrival + ")"
...
class PregnantClient(Client):
	...
	"""
		Redéfinition de la méthode publique d'auto-insertion de la cliente enceinte courant dans la file d'attente. La téte de file lui est transmise en paramétre
	"""
	def insertIntoQueue(self, headOfTheQueue):
		# S'il n'y a personne en téte de file. Autrement dit la file est vide
		if(headOfTheQueue == None):
			return self # La cliente devient la téte de file. Et c'est terminé.
		# Sinon, on parcourt les clients de la file d'attente é partir du début
		currentClient = headOfTheQueue
		# Si la téte de file n'est pas une cliente enceinte. On se met en téte de file.
		if(not isinstance(currentClient, PregnantClient)):
			currentClient.previousClient = self
			self.nextClient = currentClient
			return self
		# Sinon, on cherche la position de la derniére client enceinte
		r = currentClient
	...
	def show(self):
		# On récupére le numéro du client courant
		text = "Femme enceinte Né " + str(self.number) + " (" + self.arrival + ")\n"
		# Et on demande é l'éventuel client qui suit dans la file d'attente de s'afficher
		if(self.nextClient != None):
			text += self.nextClient.show()
		return text
...
class SeniorClient(Client):
	...
	"""
		Redéfinition de la méthode publique d'auto-insertion du client senior courant dans la file d'attente. La téte de file lui est transmise en paramétre
	"""
	def insertIntoQueue(self, headOfTheQueue):
		# S'il n'y a personne en téte de file. Autrement dit la file est vide
		if(headOfTheQueue == None):
		...
		# Sinon, on parcourt les clients de la file d'attente é partir du début
		currentClient = headOfTheQueue
		# Si la téte de file n'est pas une cliente enceinte ou un client senior. On se met en téte de file.
		...
		while(currentClient != None):
			if(isinstance(currentClient, PregnantClient) or isinstance(currentClient, SeniorClient)):
				r = currentClient
			currentClient = currentClient.nextClient
		# On se place tout auprés é trois clients derriére le dernier client senior ou la derniére clients enceinte
		# s'il y a plusieurs autres clients derriére lui
	...
	def show(self):
		# On récupére le numéro du client courant
		text = "3éme ége Né " + str(self.number) + " (" + self.arrival + ")\n"
		# Et on demande é l'éventuel client qui suit dans la file d'attente de s'afficher
		if(self.nextClient != None):
			text += self.nextClient.show()
		return text
...
class FileDattente:
	...
	def addClient(self):
		...
		# Si la file est vide, le nouveau client devient la téte de la file d'attente
	...
	def addPregnantClient(self):
		# Affectation d'un numéro de ticket é la cliente enceinte
		...
		# Si la file est vide, la nouvelle cliente devient la téte de la file d'attente
		if(self.headOfTheQueue == None):
			self.headOfTheQueue = client
	...
	def countNumberOfClients(self):
		# Initialisation du compteur du nombre de clients é zéro
		count = 0
		# On parcourt la file d'attente é partir de la téte 
	...
	def getNextClient(self):
		# On récupére l'instance de la classe <Client> en téte de la file d'attente
		topClient = self.headOfTheQueue
		# On place le deuxiéme de la file en téte
		if(topClient != None):
			self.headOfTheQueue = self.headOfTheQueue.nextClient
			self.headOfTheQueue.previousClient = None
		# On retourne le client en téte de file
		return topClient
	...
	"""
	def showQueue(self):
		# Si la file n'est pas vide, on affiche la téte de la file d'attente
		if(self.headOfTheQueue != None):
...
def isBadCommand(command):
	if((command == 'm') or (command == 'M')  # Si 'm' ou 'M' : Affichage du menu utilisateur
		or (command == 'c') or (command == 'C')  # Si 'c' ou 'C' : Ajout d'une nouveau client é la file d'attente
		or (command == 'p') or (command == 'P')  # Si 'p' ou 'P' : Ajout d'une cliente enceinte é la file d'attente
		or (command == 'o') or (command == 'O')  # Si 'o' ou 'O' : Ajout d'un client égé é la file d'attente
		or (command == 'g') or (command == 'G')  # Si 'g' ou 'G' : Récupération au client en téte de la file d'attente
...
def main():
	...
	while True :
		# Demande du choix de l'utilisateur et contréle de saisie
		while True:
			# Demande é l'utilisateur de faire un choix en tapant une commande
			command = input("-- Faites un choix: ")
			# Récupération de la premiére caractére correspondant au choix
			command = command[0]
		...
		if((command == 'c') or (command == 'C')):  # Choix d'ajout d'un client dans la file d'attente  
		...
		elif((command == 'g') or (command == 'G')): # Choix de récupération du client en téte de la file d'attente
			client = fileDattente.getNextClient() # Appel de la fonction de récupération de la téte de liste
			client.show()
			...
			print(text) # Saut de ligne é l'affichage
